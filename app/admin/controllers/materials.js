...
const materials_by_category = async (req, res, next) => {
    try {
        ...
        let [ materials, available_tests ] = await Promise.all([
            ...
            query("SELECT * FROM `vw_tests`", [])
        ]);

        // add validation to category to exists
        res.render('./admin/materials', { materials, available_tests })
    } catch (err) {
        next(err)
    }
}
...
const material_post = async (req, res, next) => {
    try {
        const { id, category_id, title, test_id, description } = req.body;
...
const material_put = async (req, res, next) => {
    try {
        ...
        const result = await query('call usp_update_learning_material(?,?,?,?,?,?,?)',
            [id, title, image, description, test_id || null, category_id, doc]
        );

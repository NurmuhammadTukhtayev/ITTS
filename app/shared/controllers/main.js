...
module.exports = async (req, res) => {
    ...
    let blogs = await query("SELECT id, title, substring(content, 1, 300) as content, author, image_url, DATE_FORMAT(published_at, '%M %e, %Y') AS published_on FROM smart_path.blog_posts order by published_at desc limit 3;");
